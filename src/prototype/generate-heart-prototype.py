"""PROTOTYPE: original stylized heart asset, MIT like this repository.

Interaction reference only; deliberately simplified shapes. No external assets.
Run with Python 3 to regenerate public/heart-prototype.glb.
"""
import json
import math
...
binary = bytearray()
gltf = {
    "asset": {"version": "2.0", "generator": "OpenEdu3D annotation prototype", "copyright": "2026 OpenEdu3D; MIT"},
...
output = Path(__file__).parent / "public" / "heart-prototype.glb"
output.parent.mkdir(exist_ok=True)
output.write_bytes(glb)
print(f"Generated {output.name}: {length:,} bytes")
